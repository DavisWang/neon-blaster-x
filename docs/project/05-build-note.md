...
- current automated coverage passes at `75/75`
...
- `Nightwing`, a late custom manta boss that sits outside the standard `28`-ship roster and uses manta-derived but more aggressive AI weighting
- a builder-side `Copy Ship Spec` action that exports the current ship as standardized `nbx-ship-design-v1` JSON for future custom enemy requests
